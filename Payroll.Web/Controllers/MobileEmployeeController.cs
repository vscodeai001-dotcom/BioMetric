...
public sealed class MobileEmployeeController : ControllerBase
{
    ...
    private static object ToPayslip(Payroll.Shared.PayrollHistory x) => new { payrollId = x.PayrollID, month = x.PayMonth, year = x.PayYear, baseSalary = x.BaseSalary ?? 0m, overtimePay = x.OvertimePay ?? 0m, bonus = x.Bonus ?? 0m, advanceDeduction = x.Deductions_Advance ?? 0m, pfDeduction = x.PfDeduction, esiDeduction = x.EsiDeduction, ptDeduction = x.PtDeduction, tdsDeduction = x.TdsDeduction, netSalary = x.NetSalary };
