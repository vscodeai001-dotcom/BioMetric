 ...
 */
window.payrollEnsureAdminJourneyCard = function (state, x, selectedId, displayPosition) {
    ...
    const statusText = arrived ? 'Within range' : (within ? 'Within range' : 'Outside range');
    const statusClass = arrived || within ? 'within' : 'outside';
    const visible = !(Number(selectedId) > 0 && Number(selectedId) !== employeeId);

    const iconHtml =
        `<div class="payroll-admin-map-employee-card ${statusClass}" data-employee-id="${employeeId}">` +
            `<div class="payroll-admin-map-card-line payroll-admin-map-card-title">` +
                `<span class="payroll-admin-map-card-person">♟</span>` +
                `<strong>${name}</strong>` +
                `<span class="payroll-admin-map-card-dot"></span>` +
                `<span class="payroll-admin-map-card-range">${statusText}</span>` +
            `</div>` +
            `<div class="payroll-admin-map-card-line payroll-admin-map-card-destination"><span>🏢</span> To Office</div>` +
            `<div class="payroll-admin-map-card-stats">` +
                `<span>📏 ${routeDistance}</span>` +
                `<span>⏱ ${eta}</span>` +
                `<span>🚦 ${window.payrollEscapeHtml(speed)}</span>` +
                `<span>🎯 ${accuracy}</span>` +
            `</div>` +
            `<div class="payroll-admin-map-card-line payroll-admin-map-card-footer"><span>📍 ${arrived ? 'Arrived' : 'GPS LIVE'}</span><span>🕐 ${elapsed}</span></div>` +
        `</div>`;

    const icon = L.divIcon({
        className: 'payroll-admin-map-employee-card-wrap',
        html: iconHtml,
        iconSize: [205, 82],
        iconAnchor: [102, 94]
    });
    ...
    const latLng = displayPosition || state.markers?.[employeeId]?.getLatLng?.();
    if (!latLng) return null;

    if (!card || !card.isConnected) {
        card = L.marker(latLng, {
            icon,
            interactive: false,
            keyboard: false,
            zIndexOffset: 1400,
        ...
        card.setIcon(icon);
        card.setLatLng(latLng);
        card.setOpacity(visible ? 1 : 0);
        if (!state.map.hasLayer(card)) card.addTo(state.map);
    }
        ...
        try {
            ...
            liveStaff.forEach(
                function (x) {
                        ...
                        window.payrollSmoothMoveMarker(
                            ...
                            function (animatedPosition) {
                                try {
                                    ...
                                    if (state.journeyCards[employeeId]) {
                                        state.journeyCards[employeeId].setLatLng(animatedPosition);
                                    }
                                }
                                catch { }
                            }
                    ...
                    if (state.journeyCards[employeeId]) {
                        state.journeyCards[employeeId].setOpacity(
                            Number(selectedId) > 0 && !isSelected ? 0 : 1
                        );
                        state.journeyCards[employeeId].setLatLng(displayPosition);
                    }
                    ...
                    // Employee journey details are rendered as one individual
                    // floating card marker per employee. No Leaflet tooltip or
                    // duplicate mini-label is used.
                    ...
                    window.payrollRequestJourneyRoute(
                        ...
                        const name = window.payrollEscapeHtml(x.name || 'Employee');
                        try { window.payrollEnsureAdminJourneyCard(state, x, selectedId, displayPosition); } catch { }
                        try { window.payrollEnsureAdminJourneyCard(state, x, selectedId, state.journeyCards[employeeId]?.getLatLng?.()); } catch { }
                    ...
                    window.payrollEnsureAdminJourneyCard(state, x, selectedId, displayPosition);
